...
public class AgileEngineImageCachingService implements ImageCachingService {
    ...
    private void generateCameraIndexes(AgileImage img) {
        Set<AgileImage> imagesByCamera = cameraIndex.getOrDefault(img.getCamera(), new HashSet<>());
        imagesByCamera.add(img);
        cameraIndex.put(img.getAuthor(), imagesByCamera);
    }

    private void generateAuthorIndexes(AgileImage img) {
        Set<AgileImage> imagesByTag = authorIndex.getOrDefault(img.getAuthor(), new HashSet<>());
        imagesByTag.add(img);
        authorIndex.put(img.getAuthor(), imagesByTag);
    }
